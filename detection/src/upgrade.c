...
{
    SDL_Surface *enlarged = SDL_CreateRGBSurface(0, surface->w * ENLARGEMENT_FACTOR, surface->h * ENLARGEMENT_FACTOR, 32, 0, 0, 0, 0);
    ...
    for (int y = 0; y < surface->h; y++)
    {
        for (int x = 0; x < surface->w; x++)
        {
            ...
            for (int dy = 0; dy < ENLARGEMENT_FACTOR; dy++)
            {
                ...
                {
                    int pos = ((y * ENLARGEMENT_FACTOR + dy) * (enlarged->pitch / 4)) + (x * ENLARGEMENT_FACTOR + dx);
...
{
    SDL_Surface *reduced = SDL_CreateRGBSurface(0, surface->w / ENLARGEMENT_FACTOR, surface->h / ENLARGEMENT_FACTOR, 32, 0, 0, 0, 0);
    ...
    for (int y = 0; y < reduced->h; y++)
    {
        ...
        {
            Uint32 *pixels = (Uint32*)surface->pixels;
            Uint32 pixel = pixels[(y * ENLARGEMENT_FACTOR) * (surface->pitch / 4) + (x * ENLARGEMENT_FACTOR)];
            SDL_GetRGBA(pixel, surface->format, &r, &g, &b, &a);
